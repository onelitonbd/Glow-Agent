...
- **File tools are workspace-confined but not read-only.** The model can create, edit, rename, delete, and overwrite files inside the project directory (never `.git`, `.env`, or database files, and never outside it — symlink escapes included). Disable **File management** in Other settings if you want read-only behaviour back.
